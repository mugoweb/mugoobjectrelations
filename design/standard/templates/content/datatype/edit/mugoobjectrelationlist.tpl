                    ...
                    {section var=Objects loop=$attribute.content.relation_list sequence=array( bglight, bgdark )}
                        {let object=fetch( content, object, hash( object_id, $Objects.item.contentobject_id ) )}
                            ...
                            <tr class="{$Objects.sequence}">
                                ...
                                <td colspan="5">
                                {if eq( $attribute.class_content.xreference_type, 'text' )}
                                    <textarea cols="80" rows="3" name="{$attribute_base}_xrefoptionaldata[{$attribute.id}][]">{$Objects.xrefoptionaldata}</textarea>
                                {else}
                                    <select name="{$attribute_base}_xrefoptionaldata[{$attribute.id}][]">
                                    {foreach $attribute.class_content.xreference_options as $option}
                                        <option value="{$option}" {if eq( $Objects.xrefoptionaldata, $option )}selected{/if}>{$option}</option>
                                    {/foreach}
                                    </select>
                                {/if}
                                </td>
